...
while True:
    try:
        ...
        #only way I could thing to do this...
        if hotdogs < 0:
            raise ValueError
    except ValueError:
        print("Please enter a number- greater than zero, without a decimal point or comma")
    except:
        print("Integer cannot have ")
    else:
        break

while True:
    try:
        chips = int(input("How many bags of chips did the customer order? "))
        # only way I could thing to do this...
        if chips < 0:
            raise ValueError
    except ValueError:
...
while True:
    try:
        soda = int(input("How many sodas did the customer order? "))
        #only way I could thing to do this...
        if soda < 0:
            raise ValueError
    except ValueError:
